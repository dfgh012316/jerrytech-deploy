...
- 自動化流程（每天 03:17 Asia/Taipei）：
  1. `check-runner`：在 runner 上比對執行中的版本與 `FROM`，不一致就失敗（代表之前 bump 了卻沒換上）。
  2. `bump`：`actions/runner` 最新 release 比 `FROM` 新，而且 base image 已上 GHCR，就改 `FROM` 與 `chart/Chart.yaml` 的 `appVersion`，以 `github-actions[bot]` commit 到 main。
...
- 失敗時 GitHub 會寄信。手動 fallback：改 `FROM`、merge（自動 build），再 `kubectl -n actions-runner rollout restart deploy/actions-runner`。
